  ...
  generate_documentation = function(source, context, callback) {
    return fs.readFile(source, "utf-8", function(error, code) {
      ...
      for (var i = 0, _len = sections.length; i < _len; i++) {
        section = sections[i];
        section.code_html = '<pre class="source-block">' + section.code_text + '</pre>'; //highlight_start + fragments[i] + highlight_end;
...
//      });
  ...
  check_config = function(context, pkg) {
    ...
    defaults = {
      ...
      prettify: __dirname + '/../resources/prettify',
      show_timestamp: true,
      output_dir: "docs",
  ...
  parse_args(function(sources, project_name, raw_paths) {
    ...
    return ensure_directory(context.config.output_dir, function() {
      ...
      exec('cp -rv ' + context.config.prettify + ' ' + context.config.output_dir, function (err) {
        if (err) throw err;
