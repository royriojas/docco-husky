...
//      });
  ...
  parse_markdown = function(context, src) {
    ...
    markdown.replace(/\[([^\[]+)\]\(([^\)]+)\)/g, function(match, c1, c2) {
      if (/\.md$/.test((c2 || '').trim())) {
        return '[' + c1 + '](' + c2 + '.html)';
      }
      return match;
  ...
  parse_args = function(args, config, callback) {
  ...
  check_config = function(context, pkg, config) {
    var defaults;
    defaults = {
  ...
  module.exports.start = function (args, config) {
    parse_args(args, config, function(sources, project_name, raw_paths) {
